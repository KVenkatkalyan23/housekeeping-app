  ...
  return (
    <span
      className={`inline-flex h-5 w-5 items-center justify-center rounded-md border ${
        active ? 'border-[#1664c0] bg-[#1664c0] text-white' : 'border-slate-300 text-slate-400'
      }`}
    >
...
interface BottomNavProps {
  activeTab: 'tasks' | 'attendance' | 'leave' | 'profile'
}
            ...
            <li key={item.label} className="flex justify-center">
              <button
                ...
                className={`flex flex-col items-center gap-1 text-[0.65rem] font-semibold uppercase tracking-[0.08em] ${
                  item.tab === activeTab ? 'text-[#1664c0]' : 'text-slate-400'
                }`}
              >
                <NavIcon active={item.tab === activeTab} />
